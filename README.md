...
3.  **Configure Environment Variables:**
    ...
    * Edit the `.env` file (or set environment variables directly) with your actual credentials:
        ...
        * `SALESFORCE_SITE`: `"https://login.salesforce.com"` (or your org domain if required).
        * `SALESFORCE_REDIRECT_URI`: `"http://localhost:4000/auth/salesforce/callback"` (must match Connected App callback URL exactly).
...
1. Copy the environment template:
   ...
   cp .env.example .env
   ```
2. Start Postgres + Phoenix app:
   ```bash
   docker compose up --build
   ```
3. Open the app at [`http://localhost:4000`](http://localhost:4000).

Notes:
* The app container runs `mix deps.get`, `mix ecto.create`, `mix ecto.migrate`, then starts Phoenix.
* Salesforce env vars are included in `.env.example` (`SALESFORCE_CLIENT_ID`, `SALESFORCE_CLIENT_SECRET`, `SALESFORCE_SITE`, `SALESFORCE_REDIRECT_URI`).
* To run without Gemini API calls in development, set `USE_FAKE_AI=true` in your `.env`. This uses a local fake AI provider for follow-up email, automation drafts, and HubSpot suggestions.
* To revert to real Gemini, set `USE_FAKE_AI=false` (or remove the variable) and restart the app.
