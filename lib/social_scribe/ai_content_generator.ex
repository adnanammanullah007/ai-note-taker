defmodule SocialScribe.AIContentGenerator do
    ...
    if is_nil(api_key) or api_key == "" do
      ...
      case Tesla.post(client(), path, payload) do
        ...
        {:ok, %Tesla.Env{status: status, body: error_body}} ->
          classify_gemini_response_error(status, error_body)
  ...
  @doc false
  def classify_gemini_response_error(status, error_body), do: classify_api_error(status, error_body)

  defp classify_api_error(status, error_body) when status in [400, 401] do
    if api_key_invalid?(error_body) do
  ...
  defp classify_api_error(status, error_body), do: {:error, {:api_error, status, error_body}}

  defp api_key_invalid?(%{"error" => error}) when is_map(error) do
    reason =
      case Map.get(error, "details") do
        details when is_list(details) ->
          Enum.flat_map(details, fn detail ->
            if is_map(detail), do: [Map.get(detail, "reason")], else: []
          end)
    ...
    Enum.member?(reason, "API_KEY_INVALID") || String.contains?(message, "api key not valid")
  end

  defp api_key_invalid?(_), do: false
end
