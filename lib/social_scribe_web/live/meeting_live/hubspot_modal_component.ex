defmodule SocialScribeWeb.MeetingLive.HubspotModalComponent do
  ...
  alias SocialScribe.Crm.Provider, as: CrmProvider
  ...
  def render(assigns) do
    ...
    <div class="space-y-6">
      <div>
        <h2 id={"#{@modal_id}-title"} class="text-xl font-medium tracking-tight text-slate-900">{@modal_title}</h2>
  ...
  def update(assigns, socket) do
    socket =
      ...
      |> assign_new(:provider, fn -> assigns.credential.provider end)
      |> assign_new(:provider_name, fn -> CrmProvider.display_name(assigns.credential.provider) end)
      |> assign_new(:modal_title, fn -> CrmProvider.modal_title(assigns.credential.provider) end)
      |> assign_new(:submit_text, fn -> CrmProvider.submit_text(assigns.credential.provider) end)
  ...
  def handle_event("contact_search", %{"value" => query}, socket) do
    ...
    if String.length(query) >= 2 do
      ...
      send(self(), {:crm_search, query, socket.assigns.credential})
      {:noreply, socket}
    else
  ...
  def handle_event("apply_updates", %{"apply" => selected, "values" => values}, socket) do
    ...
    send(self(), {:apply_crm_updates, updates, socket.assigns.selected_contact, socket.assigns.credential})
    {:noreply, socket}
  end
