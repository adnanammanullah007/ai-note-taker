defmodule SocialScribeWeb.MeetingLive.Show do
  ...
  @impl true
  def handle_info({:crm_search, query, credential}, socket) do
  ...
  def handle_info({:generate_suggestions, contact, meeting, credential}, socket) do
    case CrmSuggestions.generate_suggestions_from_meeting(meeting) do
      {:ok, suggestions} ->
        merged =
          CrmSuggestions.merge_with_contact(
            suggestions,
        ...
        send_update(SocialScribeWeb.MeetingLive.HubspotModalComponent,
          ...
          provider_name: CrmProvider.display_name(credential.provider),
          submit_text: CrmProvider.submit_text(credential.provider)
        )
  ...
  def handle_info({:apply_crm_updates, updates, contact, credential}, socket) do
    case Crm.update_contact(credential, contact.id, updates) do
      {:ok, _updated_contact} ->
        provider_name = CrmProvider.display_name(credential.provider)

        socket =
          socket
          |> put_flash(:info, "Successfully updated #{map_size(updates)} field(s) in #{provider_name}")
